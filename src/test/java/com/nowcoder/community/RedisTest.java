...
public class RedisTest {
    ...
    public void testHyperLogLog() {
        String redisLey = "test:hll:01";
        for (int i = 1; i <= 10000; i ++) {
            redisTemplate.opsForHyperLogLog().add(redisLey, i);
        }
        for (int i = 1; i <= 10000; i ++) {
            int r = (int) (Math.random() * 10000 + 1);
            redisTemplate.opsForHyperLogLog().add(redisLey, r);
        }
        // 统计结果
        System.out.println(redisTemplate.opsForHyperLogLog().size(redisLey));
    }
    ...
    public void testBitmap() {
        ...
        redisTemplate.opsForValue().setBit(redisKey, 8, false);
        ...
        Object obj = redisTemplate.execute((RedisCallback) redisConnection ->
                redisConnection.bitCount(redisKey.getBytes()));
        System.out.println(obj);
    }

    // 统计3组数据的布尔值，并对着3组数据做运算
    @Test
    public void testBitMap() {
