...
//        return userMapper.selectById(id);
    ...
    public Map<String, Object> register(User user) {
        ...
        //验证账号
        User foundUser = userMapper.selectByName(user.getUsername());
        if (foundUser != null) {
            map.put("usernameMsg", "该账号已经存在");
            return map;
        }
        //验证邮箱
        foundUser = userMapper.selectByEmail(user.getEmail());
        if (foundUser != null) {
    ...
    public Map<String, Object> login(String username, String password, int expiredSeconds) {
        ...
        LoginTicket loginTicket = LoginTicket.builder().userId(user.getId())
                .ticket(CommunityUtil.genetateUUID())
                .status(0)
                .expired(new Date(System.currentTimeMillis() + 1000 * expiredSeconds)).build();
